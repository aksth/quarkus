...
import io.swagger.model.MapStringList;
...
public class ArticleApiController implements ArticleApi {
    ...
    public ResponseEntity<MapStringList> articleGet() {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<MapStringList>(objectMapper.readValue("{\n  \"key\" : {\n    \"empty\" : true\n  }\n}", MapStringList.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
                return new ResponseEntity<MapStringList>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<MapStringList>(HttpStatus.NOT_IMPLEMENTED);
    }
